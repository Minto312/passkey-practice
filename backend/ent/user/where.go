...
func HasPasskeys() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, PasskeysTable, PasskeysPrimaryKey...),
		)
...
// HasAuthHistories applies the HasEdge predicate on the "auth_histories" edge.
...
// HasAuthHistoriesWith applies the HasEdge predicate on the "auth_histories" edge with a given conditions (other predicates).
func HasAuthHistoriesWith(preds ...predicate.AuthHistory) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newAuthHistoriesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
