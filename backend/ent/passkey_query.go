...
func (pq *PasskeyQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (pq *PasskeyQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (pq *PasskeyQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (pq *PasskeyQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (pq *PasskeyQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (pq *PasskeyQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := pq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
//		GroupBy(passkey.FieldCredentialID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (pq *PasskeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Passkey, error) {
	...
	if query := pq.withUser; query != nil {
		if err := pq.loadUser(ctx, query, nodes,
			func(n *Passkey) { n.Edges.User = []*User{} },
			func(n *Passkey, e *User) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PasskeyQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Passkey, init func(*Passkey), assign func(*Passkey, *User)) error {
	...
	byID := make(map[uuid.UUID]*Passkey)
	nids := make(map[uuid.UUID]map[*Passkey]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(passkey.UserTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(passkey.UserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(passkey.UserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(passkey.UserPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Passkey]struct{}{byID[outValue]: {}}
