...
type UserQuery struct {
	...
	withAuthHistories *AuthHistoryQuery
...
// QueryAuthHistories chains the current query on the "auth_histories" edge.
func (uq *UserQuery) QueryAuthHistories() *AuthHistoryQuery {
	query := (&AuthHistoryClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// the "auth_histories" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithAuthHistories(opts ...func(*AuthHistoryQuery)) *UserQuery {
	query := (&AuthHistoryClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAuthHistories; query != nil {
		if err := uq.loadAuthHistories(ctx, query, nodes,
			func(n *User) { n.Edges.AuthHistories = []*AuthHistory{} },
			func(n *User, e *AuthHistory) { n.Edges.AuthHistories = append(n.Edges.AuthHistories, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadPasskeys(ctx context.Context, query *PasskeyQuery, nodes []*User, init func(*User), assign func(*User, *Passkey)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.PasskeysTable)
		s.Join(joinT).On(s.C(passkey.FieldID), joinT.C(user.PasskeysPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.PasskeysPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.PasskeysPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Passkey](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "passkeys" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadAuthHistories(ctx context.Context, query *AuthHistoryQuery, nodes []*User, init func(*User), assign func(*User, *AuthHistory)) error {
	...
	query.withFKs = true
	query.Where(predicate.AuthHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AuthHistoriesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_auth_histories" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_auth_histories" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
